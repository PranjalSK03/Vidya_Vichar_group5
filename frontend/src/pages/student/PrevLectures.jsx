...
const PrevLectures = () => {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const res = await fetch('/api/student/dashboard/enrolled-courses', {
  ...
  useEffect(() => {
    ...
    const fetchLectures = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('/api/student/dashboard/prev-lectures', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
        if (data.success) {
      ...
      <ul>
        {lectures.map(lec => (
          <li key={lec._id}>
            ...
            <PrevLectureQuestions lectureId={lec.lecture_id} />
          </li>
        ))}
      </ul>
    </div>
...
const PrevLectureQuestions = ({ lectureId }) => {
        ...
        <ul>
          {questions.map(q => (
            <li key={q._id}>
              ...
              <PrevLectureAnswers questionId={q._id} />
            </li>
          ))}
        </ul>
      )}
...
const PrevLectureAnswers = ({ questionId }) => {
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`/api/student/dashboard/all-answers/${questionId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
        if (data.success) {

...
const teacherController = {
  ...
  makeStudentTA: async (req, res) => {//POST type
    try {
      const { student_id, course_id, roll_no } = req.body;
      if ((!student_id && !roll_no) || !course_id) {
        return res.status(400).json({
          success: false,
          message: 'student_id (or roll_no) and course_id are required in the request body.'
        });
      }

      // Find the student by student_id or roll_no
      let student;
      if (student_id) {
  ...
  getLecturesForCourse: async (req, res) => {
    try {
      const { course_id } = req.params;
      if (!course_id) {
        return res.status(400).json({ success: false, message: 'course_id is required as a URL parameter.' });
      }
      // Find the course and verify teacher has access
      const teacher = await Teacher.findById(req.user.id);
      if (!teacher) {
        return res.status(404).json({ success: false, message: 'Teacher not found.' });
      }
      ...
      const lectures = await Lecture.find({ course_id }).sort({ class_start: 1 });
      res.status(200).json({
        success: true,
        data: {
          lectures: lectures.map(l => ({
            ...
            class_start: l.class_start,
            class_end: l.class_end,
            lec_num: l.lec_num
          }))
        }
      });
